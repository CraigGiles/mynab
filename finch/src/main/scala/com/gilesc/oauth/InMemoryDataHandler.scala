...
object InMemoryDataHandler extends DataHandler[OAuthUser] {
  ...
  private[this] val clients = List[AuthData](
    AuthData(
      "myusername",
      "mypassword",
      MynabSettings.clientId,
      "user_secret",
      "user_auth_code",
      OAuthUser("user", "Craig Giles")
    ),
    AuthData(
      "user_name",
      "user_password",
  ...
  private[this] val accessTokens = new ConcurrentHashMap[ClientId, AccessToken]().asScala
  private[this] val authInfosByAccessToken = new ConcurrentHashMap[Token, AuthInfo[OAuthUser]]().asScala
